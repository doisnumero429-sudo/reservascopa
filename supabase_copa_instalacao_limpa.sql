-- ============================================================================
-- ARAÇÁ GRILL · INSTALAÇÃO LIMPA DO BANCO DE RESERVAS DA COPA
-- Evento: Escócia x Brasil · 24/06/2026 · 19h
--
-- ATENÇÃO:
-- Este arquivo recria SOMENTE as estruturas cujo nome começa com copa_ e o
-- bucket copa-comprovantes. Ele foi feito para a instalação antes da divulgação.
-- Por segurança, a execução é interrompida se encontrar reservas ou comprovantes.
-- ============================================================================

-- Interrompe antes de apagar qualquer coisa caso já existam dados reais.
do $$
begin
  if to_regclass('public.copa_reservas') is not null
     and exists (select 1 from public.copa_reservas limit 1) then
    raise exception 'Existem reservas na tabela copa_reservas. Não use a instalação limpa; faça backup ou remova apenas os testes.';
  end if;

  if exists (
    select 1 from storage.objects where bucket_id = 'copa-comprovantes' limit 1
  ) then
    raise exception 'Existem arquivos no bucket copa-comprovantes. Não use a instalação limpa antes de fazer backup.';
  end if;
end;
$$;

-- Remove funções antigas e atuais deste projeto, independentemente da assinatura.
do $$
declare
  r record;
begin
  for r in
    select p.oid::regprocedure as assinatura
     ...
     where n.nspname = 'public'
       and p.proname in (
         'copa_set_atualizado_em',
         'copa_limpar_bloqueios_expirados',
         'copa_limpar_bloqueios_expirados_v2',
         'listar_mesas_ocupadas_copa',
         'listar_mesas_ocupadas_copa_v2',
         'criar_reserva_copa',
         'criar_reserva_copa_v2',
         'registrar_comprovante_copa_v2',
         'cancelar_reserva_copa_cliente_v2',
         'confirmar_reserva_copa_admin',
         'cancelar_reserva_copa_admin'
       )
  loop
...
drop view if exists public.copa_reservas_admin cascade;
drop table if exists public.copa_reserva_historico cascade;
drop table if exists public.copa_mesas_bloqueios cascade;
drop table if exists public.copa_reserva_mesas cascade;
drop table if exists public.copa_reservas cascade;
drop table if exists public.copa_mesas cascade;
drop table if exists public.copa_eventos cascade;

delete from storage.buckets where id = 'copa-comprovantes';
